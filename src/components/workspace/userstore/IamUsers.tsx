...
const IamUsers = () => {
  ...
  const fetchDetails = async () => {
    ...
    if (response) {
      ...
      return response.users; // ✅ Return the list
    }
  ...
  useEffect(() => {
    if (searchUserTerm.length >= 3) {
      ...
      if (hasSearchedOnce.current) {
        fetchDetails(); // only re-fetch if user previously searched
      }
    }
  }, [searchUserTerm]);
  ...
  useEffect(() => {
    if (selectedUser) {
      const space = Number(selectedUser.availableSpace); // Force numeric conversion
  ...
  return (
    <div className="">
      <div className="grid grid-cols-3">
        ...
        <div className="col-span-1 flex flex-row items-center h-full justify-center flex  gap-2 border dark:border-zinc-800 p-2 rounded-tr-xl border-l-0 border-b-0">
      ...
      <div className="grid grid-cols-3">
        <div
          className="col-span-2 flex flex-col gap-2 border dark:border-zinc-800 p-2 border-t-0 
       max-h-[calc(100dvh-286px)] overflow-y-auto scrollbar-hide
        "
        >
          ...
          {selectedUser ? (
            <div className="flex flex-col  gap-2 text-sm">
              ...
              <Input
                ...
                type="number"
                min={0}
                value={String(editableStorage)} // Convert to string for value
                onChange={(e) => setEditableStorage(Number(e.target.value))} // Convert back to number
                endContent="GB"
              />
            </div>
          ) : (
              ...
              <Button
                ...
                onPress={async () => {
                  ...
                  const body = {
                    ...
                    actionType,
                    enabled,
                    space: selectedUser.availableSpace, // preserve storage
                  };
                      ...
                      addToast({
                        title:
                          actionType === "block"
                            ? t("userBlocked")
                            : t("userUnblocked"),
                        color: actionType === "block" ? "success" : "success",
                      });

                      // ✅ Refetch the full user list

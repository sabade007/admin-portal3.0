...
const SuiteAppsMob = () => {
  ...
  const { currentMobileTab, getAllApplications } = useWorkspaceStore();
  ...
  const filteredApps =
    currentMobileTab === "suitemob" && searchTermmob.length >= 3
  ...
  const AppCard = ({
    ...
    return (
      <motion.div
        whileHover="hover"
        initial="initial"
        className={`relative dark:bg-zinc-800 h-auto p-3 gap-2 shadow-sm rounded-2xl  group cursor-pointer  transition-shadow flex flex-col items-center justify-center`}
        onClick={() => {

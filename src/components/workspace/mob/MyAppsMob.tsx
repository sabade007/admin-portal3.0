...
const MyAppsMob = () => {
  ...
  const { applicationArray, currentMobileTab, bookMarks, getAllApplications } =
    useWorkspaceStore();
  const { themeType } = useThemetypeStore();
  const themeColor = useThemeColor();
  const { fontsize } = useFontSizeStore();
  const { searchTermmob } = useSearchStore();
  ...
  const filteredApps =
    currentMobileTab === "myappsmob" && searchTermmob.length >= 3
      ? nonDefaultApps.filter((app: any) =>
          app.applicationName
            .toLowerCase()
            .includes(searchTermmob.toLowerCase())
        )
      : nonDefaultApps;
  ...
  const filteredBookmarks =
    currentMobileTab === "myappsmob" && searchTermmob.length >= 3
      ? bookMarks.filter((bookmark: any) =>
          bookmark.name.toLowerCase().includes(searchTermmob.toLowerCase())
        )
      : bookMarks;
  ...
  const AppCard = ({
    ...
    return (
      <motion.div
        whileHover="hover"
        initial="initial"
        className={` relative h-auto p-1 gap-2 shadow-sm rounded-2xl group cursor-pointer transition-shadow flex flex-col items-center justify-center`}
        onClick={() => {
